...
public class SceneGraph extends JPanel {
	...
	private void createWorld() {
		...
		triangle2.setScale(0.75, 1.875).setTranslation(-2, -0.5).setColor(Color.MAGENTA);
		...
		polygon1 = new TransformedObject(filledPolygon);
		polygon1.setScale(0.8, 0.8).setTranslation(-1.25, 0).setColor(Color.ORANGE);
		polygon2 = new TransformedObject(filledPolygon);
		polygon2.setScale(0.8, 0.8).setTranslation(1.25, -1).setColor(Color.ORANGE);

		polygon3 = new TransformedObject(filledPolygon);
		polygon3.setScale(0.6, 0.6).setTranslation(-3, 1.8).setColor(Color.ORANGE);
		polygon4 = new TransformedObject(filledPolygon);
		polygon4.setScale(0.6, 0.6).setTranslation(-1, 1).setColor(Color.ORANGE);

		polygon5 = new TransformedObject(filledPolygon);
		polygon5.setScale(0.4, 0.4).setTranslation(1.3, 2.05).setColor(Color.ORANGE);
	...
	private static SceneGraphNode filledPolygon = new SceneGraphNode() {
		void doDraw(Graphics2D g) { // width = 1, height = 1, center of base is at (0,0);
			...
			for (int i = 1; i <= 13; i++) {
				if (i == 1) {
					path.moveTo((0.5 * Math.cos((2 * Math.PI / 13) * i)), (0.5 * Math.sin((2 * Math.PI / 13) * i)));
				} else {
					path.lineTo((0.5 * Math.cos((2 * Math.PI / 13) * i)), (0.5 * Math.sin((2 * Math.PI / 13) * i)));
				}
			}
