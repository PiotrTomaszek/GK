 ...
 * subroutines to implement hierarchical modeling. There is a check box that
...
//		rotatingRect(g2);  // (DELETE THIS EXAMPLE)
		...
		myFilledTriangle(g2);
		myFilledTriangle2(g2);
		myFilledTriangle3(g2);
	...
	private void rotatingRect(Graphics2D g2) {
		AffineTransform saveTransform = g2.getTransform(); // (It might be necessary to save/restore transform and
															// color)
	...
	private void rotatingRect2(Graphics2D g2) {
		AffineTransform saveTransform = g2.getTransform(); // (It might be necessary to save/restore transform and
															// color)
		Color saveColor = g2.getColor();
		g2.setColor(Color.RED);
		g2.rotate(Math.toRadians(frameNumber * 0.5), -2.125, 1);
		myFilledRect2(g2);
		g2.setColor(saveColor);
		g2.setTransform(saveTransform);
	}
	...
	private static void myFilledRect(Graphics2D g2) { // Fills a square, size = 1, center = (0,0)
		g2.fill(new Rectangle2D.Double(-2, -0.125, 4, 0.25));
	}

	private static void myFilledRect2(Graphics2D g2) { // Fills a square, size = 1, center = (0,0)
		g2.fill(new Rectangle2D.Double(-3.125, 0.875, 2, 0.125));
	}

	private static void myFilledRect3(Graphics2D g2) { // Fills a square, size = 1, center = (0,0)
		g2.fill(new Rectangle2D.Double(-2, -0.125, 4, 0.25));
	}

	private static void myFilledTriangle(Graphics2D g2) {
	...
	private static void myFilledTriangle2(Graphics2D g2) {
		Path2D path = new Path2D.Double();
		path.moveTo(-2, -1);
		path.lineTo(-2.25, -1);
		path.lineTo(-2.125, 1);
	...
	private static void myFilledTriangle3(Graphics2D g2) {
		Path2D path = new Path2D.Double();
		path.moveTo(2, 0.125);
		path.lineTo(2.2, 0.125);
		path.lineTo(2.1, 1.125);
		path.closePath();
